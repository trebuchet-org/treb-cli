...
const (
	// DefaultSubmodulePath is the default path to the treb-sol submodule
	DefaultSubmodulePath = "lib/treb-sol"
)
...
func (m *TrebSolManager) WithSubmodulePath(path string) *TrebSolManager {
	m.submodulePath = path
	return m
}

// shortCommit returns a short version of a commit hash (first 7 chars)
// or the full hash if it's shorter than 7 chars
...
func (m *TrebSolManager) GetCurrentCommit() (string, error) {
	cmd := exec.Command("git", "submodule", "status", m.submodulePath)
	...
	for _, line := range lines {
		if strings.Contains(line, m.submodulePath) {
	...
	return "", fmt.Errorf("could not find %s in submodule status", m.submodulePath)
}
...
func (m *TrebSolManager) CheckIfCommitExists(commit string) (bool, error) {
	cmd := exec.Command("git", "-C", m.submodulePath, "cat-file", "-e", commit)
	cmd.Dir = m.projectRoot
...
func (m *TrebSolManager) CheckoutCommit(commit string) error {
	...
	fetchCmd := exec.Command("git", "-C", m.submodulePath, "fetch", "origin")
	...
	checkoutCmd := exec.Command("git", "-C", m.submodulePath, "checkout", commit)
...
func (m *TrebSolManager) IsTrebSolInstalled() bool {
	trebSolPath := filepath.Join(m.projectRoot, m.submodulePath)
