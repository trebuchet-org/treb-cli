...
// ForgeExecutor handles Forge command execution with streaming output
...
func (f *ForgeExecutor) Build() error {
...
func (f *ForgeExecutor) Run(opts ScriptOptions) (*ScriptResult, error) {
	...
	parsedOutput := &domain.ForgeScriptOutput{
		ConsoleLogs:  []string{},
		TraceOutputs: []domain.ForgeTraceOutput{},
		Stages:       []domain.ForgeExecutionStage{},
	}
		...
		case "Receipt":
			if receipt, ok := entity.Data.(Receipt); ok {
				if parsedOutput.Receipts == nil {
					parsedOutput.Receipts = []domain.ForgeReceipt{}
				}
	...
	// Add stages from processor
	if processor != nil {
		parsedOutput.Stages = convertStageInfo(processor.stages)
	}
...
func (f *ForgeExecutor) buildArgs(opts ScriptOptions) []string {
	args := []string{"script", opts.Script, "--ffi"}
...
func (f *ForgeExecutor) buildEnv(opts ScriptOptions) []string {
	var env []string
...
func (f *ForgeExecutor) ParseOutput(output []byte) (*domain.ForgeScriptOutput, error) {
	result := &domain.ForgeScriptOutput{}
...
func (f *ForgeExecutor) extractConsoleLogs(logs []string) []string {
...
func (f *ForgeExecutor) extractConsoleLogsFromText(textOutput string) []string {
	var consoleLogs []string
...
func (f *ForgeExecutor) saveDebugOutput(output []byte) {
	runUUID := fmt.Sprintf("%d", time.Now().Unix())
	debugDir := filepath.Join(f.projectRoot, "out", ".treb-debug", runUUID)
...
// convertStageInfo converts internal StageInfo to domain.ForgeExecutionStage
func convertStageInfo(stages []StageInfo) []domain.ForgeExecutionStage {
	result := make([]domain.ForgeExecutionStage, len(stages))
	for i, stage := range stages {
		result[i] = domain.ForgeExecutionStage{
