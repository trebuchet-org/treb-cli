...
type Resolver struct {
	networks      map[string]*domain.NetworkInfo
...
func NewResolver() *Resolver {
	r := &Resolver{
		networks:      make(map[string]*domain.NetworkInfo),
...
func (r *Resolver) initializeDefaultNetworks() {
	defaultNetworks := []domain.NetworkInfo{
...
func (r *Resolver) addNetwork(network *domain.NetworkInfo) {
...
func (r *Resolver) LoadNetworks(networks map[string]*domain.NetworkInfo) {
...
func (r *Resolver) ResolveNetwork(ctx context.Context, input string) (*domain.NetworkInfo, error) {
	// Empty input
	if input == "" {
...
func (r *Resolver) GetNetworkByChainID(chainID uint64) (*domain.NetworkInfo, error) {
	if name, ok := r.chainIDLookup[chainID]; ok {
		return r.networks[name], nil
	}
...
func (r *Resolver) ListNetworks() []*domain.NetworkInfo {
...
func (r *Resolver) LoadFromConfig(config *NetworkConfig) error {
	...
	for name, rpcURL := range config.RPCEndpoints {
		if network, ok := r.networks[name]; ok {
			...
			r.addNetwork(&domain.NetworkInfo{
				Name:   name,
				RPCURL: rpcURL,
			})
		}
