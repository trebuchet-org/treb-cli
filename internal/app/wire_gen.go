...
func InitApp(v *viper.Viper, cmd *cobra.Command) (*App, error) {
	...
	foundryProfile := adapters.ProvideFoundryProfile(runtimeConfig)
	repository := contracts.NewRepository(string2, foundryProfile, logger)
	...
	restartFork := usecase.NewRestartFork(runtimeConfig, forkStateStoreAdapter, forkFileManagerAdapter, manager, forgeAdapter)
